...
PKG_SHORT="Samba_AD"			# -> pfSense-pkg-Samba_AD
PKG_TITLE="Samba AD"			# name shown in the Package Manager
PKG_VERSION="1.1.0"
PKG_CATEGORY="net"
PKG_LICENSE="BSD2CLAUSE"
PKG_COMMENT="Active Directory domain membership for pfSense"
PKG_DESCR="Joins pfSense to an Active Directory domain using Samba winbind, so Squid and other services can authenticate against the directory."
PKG_CONFIGFILE="samba_ad.xml"
SRC_DIRNAME="samba-ad"
...
LEGACY_NAMES="sambaad"

FILES_PKG="samba_ad.inc samba_ad.xml squid_ad_patch.php"
FILES_WWW="diag_samba_ad.php"
FILES_PRIV="samba_ad.priv.inc"
FILES_BIN=""
...
resolve_bin_deps() {
	# Honour an explicit choice: SAMBA_PKG=samba419 ./install.sh install
	if [ -n "${SAMBA_PKG:-}" ]; then echo "${SAMBA_PKG}"; return; fi

	# Prefer what is already installed, so reinstalling never moves the box
	# to a different Samba major version behind the operator's back.
	_i="$(pkg info -q 2>/dev/null | grep -E '^samba[0-9]+-' | head -1 | sed 's/-[0-9].*$//')"
	[ -n "${_i}" ] && { echo "${_i}"; return; }

	# status runs without network access.
	[ "${1:-}" = "offline" ] && return 0

	# Otherwise take the newest major version the repository offers.
	tpkg search -q '^samba[0-9]' 2>/dev/null | sed 's/-[0-9].*$//' | sort -Vr | head -1
}
