...
\$found = false;
foreach (\$pkgs as \$p) {
	if (isset(\$p['name']) && \$p['name'] === 'e2guardian') { \$found = true; break; }
}
if (!\$found) {
	\$pkgs[] = [
		'name'              => 'e2guardian',
		'descr'             => 'Web content filtering: inspects response bodies, phrases and MIME types.',
		'website'           => 'https://github.com/bootablearg/pfsense-packages-revived',
		'version'           => '1.0.0',
		'configurationfile' => 'e2guardian.xml',
	];
}
...
write_config('E2guardian Proxy: register with Package Manager');
exec;
exit
PKGEOF
}
...
\$pkgs = array_values(array_filter(\$pkgs, function (\$p) {
	return !(isset(\$p['name']) && \$p['name'] === 'e2guardian');
}));
...
write_config('E2guardian Proxy: unregister from Package Manager');
exec;
exit
PKGEOF
}
