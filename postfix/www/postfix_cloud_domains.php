  ...
  if (file_exists($postfix_dir.'/'.$postfix_db.".db")){
	// noqueue
        ...
        foreach($row_noqueue as $i => $row){
				...
				if (!is_executable("/usr/local/bin/spf-tools/despf.sh")) {
				   print "spf-tools is not installed; cannot expand SPF for $domain\n";
				} else if (preg_match("/\w+/",$domain)) {
				   $spf = "spf_" . $domain;
				   $$spf = array();
                                   exec("/usr/local/bin/spf-tools/despf.sh $domain",$$spf);
                                   /* was: count($$spf > 1) -- the parenthesis
                                    * sat in the wrong place, so this counted a
                                    * boolean. Fatal under PHP 8. */
                                   if (count($$spf) > 1) {
                                        $reload_postfix++;
                                        $domains[]=$domain;
