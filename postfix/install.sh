...
\$found = false;
foreach (\$pkgs as \$p) {
	if (isset(\$p['name']) && \$p['name'] === 'postfix') { \$found = true; break; }
}
if (!\$found) {
	\$pkgs[] = [
		'name'              => 'postfix',
		'descr'             => 'Mail gateway and forwarder: SMTP, antispam and relay.',
		'website'           => 'https://github.com/bootablearg/pfsense-packages-revived',
		'version'           => '1.0.0',
		'configurationfile' => 'postfix.xml',
	];
}
...
write_config('Postfix Forwarder: register with Package Manager');
exec;
exit
PKGEOF
}
...
\$pkgs = array_values(array_filter(\$pkgs, function (\$p) {
	return !(isset(\$p['name']) && \$p['name'] === 'postfix');
}));
...
write_config('Postfix Forwarder: unregister from Package Manager');
exec;
exit
PKGEOF
}
